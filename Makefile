...
# --- build targets ---
$(BUILDDIR)/wow.com: $(OBJS) $(TLS_LIB) $(SSL_ZIP)
	$(CC) $(CFLAGS) -o $@ $(OBJS) $(TLS_LIB)
	$(ZIPCOPY) $(SSL_ZIP) $@
...
$(BUILDDIR)/tls_test.com: tests/tls_test.c $(TEST_HTTP_OBJS) $(TLS_LIB) $(SSL_ZIP) | $(BUILDDIR)
	$(CC) $(CFLAGS) -Iinclude -Ivendor/cjson -o $@ $< $(TEST_HTTP_OBJS) $(TLS_LIB)
	$(ZIPCOPY) $(SSL_ZIP) $@

$(BUILDDIR)/registry_test.com: tests/registry_test.c $(TEST_HTTP_OBJS) $(TLS_LIB) $(SSL_ZIP) | $(BUILDDIR)
	$(CC) $(CFLAGS) -Iinclude -Ivendor/cjson -o $@ $< $(TEST_HTTP_OBJS) $(TLS_LIB)
	$(ZIPCOPY) $(SSL_ZIP) $@

$(BUILDDIR)/ruby_mgr_test.com: tests/ruby_mgr_test.c $(TEST_HTTP_OBJS) $(TLS_LIB) $(SSL_ZIP) | $(BUILDDIR)
	$(CC) $(CFLAGS) -Iinclude -Ivendor/cjson -o $@ $< $(TEST_HTTP_OBJS) $(TLS_LIB)
	$(ZIPCOPY) $(SSL_ZIP) $@
...
test: test-tls test-registry test-ruby-mgr
...
.PHONY: clean distclean test test-tls test-registry test-ruby-mgr
